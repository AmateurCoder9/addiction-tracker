...
export default function HeatmapGrid({ logs }: HeatmapGridProps) {
    ...
    function getBg(s: string | null): string {
        if (s === "clean") return "bg-neutral-600";
        if (s === "relapse") return "bg-white";
    ...
    return (
        <div className="p-6 rounded-lg bg-neutral-950 border border-neutral-800">
            <h3 className="text-sm font-medium text-neutral-300 mb-4">Heatmap</h3>
            <div className="space-y-2 overflow-x-auto">
                {data.map((month) => (
                    <div key={month.label} className="flex items-center gap-2">
                        <span className="text-[0.65rem] text-neutral-600 w-12 shrink-0 text-right font-mono">{month.label}</span>
                        <div className="flex gap-px">
                            {month.days.map((d) => (
                                <div key={d.date} className={`w-2.5 h-2.5 rounded-sm ${getBg(d.status)} hover:ring-1 hover:ring-neutral-500 transition-all`} title={`${d.date}${d.status ? ` — ${d.status}` : ""}`} />
                            ))}
                        </div>
                    </div>
                ))}
