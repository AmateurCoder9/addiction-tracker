...
const COLORS = { clean: "#a3a3a3", relapse: "#404040", partial: "#d4d4d4" };
    ...
    return (
        <div className="space-y-4">
            <div className="card p-5">
                <h3 className="text-sm font-medium text-neutral-900 mb-4">Monthly — {addictionName}</h3>
                <div className="h-56">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={barData} margin={{ top: 5, right: 5, left: -25, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f5f5f5" />
                            <XAxis dataKey="name" tick={{ fill: "#a3a3a3", fontSize: 11 }} axisLine={{ stroke: "#e5e5e5" }} />
                            <YAxis tick={{ fill: "#a3a3a3", fontSize: 11 }} axisLine={{ stroke: "#e5e5e5" }} allowDecimals={false} />
                            <Tooltip contentStyle={{ background: "#fff", border: "1px solid #e5e5e5", borderRadius: "0.5rem", fontSize: "0.75rem" }} />
                            ...
                            <div className="space-y-2">
                                {pieData.map((e) => (
                                    <div key={e.name} className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs text-neutral-500">{e.name}: <span className="text-neutral-900 font-medium">{e.value}</span></span>
                                    </div>
                                ))}
                            </div>
                        </div>
